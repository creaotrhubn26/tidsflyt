...
// forsvare seg mot. Sikkerhetsherdingen krever nå i tillegg eksplisitt
// ALLOW_DEV_AUTH_BYPASS=true. isAuthenticated sin bypass gjør bare `return
// next()` UTEN å røre req.user, så den nedenstående middlewarens injiserte
// req.user overlever uendret — nøyaktig det testen trenger.
...
describe("oppgavetildeling: POST /api/tasks + GET /api/tasks/assignable-colleagues", () => {
  const originalDevAuthBypass = process.env.ALLOW_DEV_AUTH_BYPASS;
  ...
  afterAll(() => {
    if (originalDevAuthBypass === undefined) {
      ...
      process.env.ALLOW_DEV_AUTH_BYPASS = originalDevAuthBypass;
    }
  });
