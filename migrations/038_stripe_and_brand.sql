...
UPDATE tidum_salg_contract_templates
...
ALTER TABLE tidum_pricing_tiers
...
CREATE INDEX IF NOT EXISTS idx_pricing_tiers_stripe_product
  ON tidum_pricing_tiers(stripe_product_id);

-- ============= 4. STRIPE-INNSTILLINGER =============

INSERT INTO tidum_salg_settings (key, value, data_type, category, label, description, sort_order, is_secret)
VALUES
...
CREATE TABLE IF NOT EXISTS tidum_stripe_events (
  ...
  revenue_event_id INTEGER REFERENCES tidum_revenue_events(id)
);

CREATE INDEX IF NOT EXISTS idx_stripe_events_type ON tidum_stripe_events(event_type);
CREATE INDEX IF NOT EXISTS idx_stripe_events_customer ON tidum_stripe_events(customer_id);
CREATE INDEX IF NOT EXISTS idx_stripe_events_subscription ON tidum_stripe_events(subscription_id);
