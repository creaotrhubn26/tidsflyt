...
| **G-10** | **Sikkerhetsherding** (uavhengig av anbudet): dev-mode auth-bypass (`server/middleware/auth.ts:26-29`), JWT-fallback-hemmelighet, PowerOffice-nøkler/SMTP-passord i klartekst, ingen helmet/CSP/HSTS, ingen CSRF-vern, ingen RLS i Postgres, inkonsistent vendor-scoping (`server/routes/export-routes.ts` mangler vendor-filter), ingen 2FA/MFA | Vil bli funnet i enhver sikkerhetsrevisjon | — | Må ryddes før tilbud på sensitiv sektor er troverdig |
