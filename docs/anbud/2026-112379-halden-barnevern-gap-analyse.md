...
| **G-10** | **Sikkerhetsherding** (uavhengig av anbudet): historisk dev-mode auth-bypass, fallback-hemmeligheter, uforseglet PowerOffice ClientKey, manglende helmet/CSP/HSTS og CSRF, ingen tilpasset RLS, inkonsistent vendor-scoping og ingen 2FA/MFA | Vil bli funnet i enhver sikkerhetsrevisjon | Integrasjonsgrenen har lukket dev-bypass/fallback, transportkontroller, brede BOLA-flater, forseglet/roterbar PowerOffice ClientKey og RLS fase 1–3B for bekymringsmeldingskjernen, sikker dialog, arkiv og frister med trygg mottakerbinding; produksjonshvelv, egen runtime-identitet, resten av RLS-/endepunktsmatrisen, kommunalt «need-to-know», MFA og ekstern test gjenstår | Resten må ryddes og kontrollene produksjonsbevises før tilbud på sensitiv sektor er troverdig |
