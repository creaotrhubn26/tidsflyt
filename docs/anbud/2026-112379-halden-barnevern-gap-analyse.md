...
| **G-10** | **Sikkerhetsherding** (uavhengig av anbudet): historisk dev-mode auth-bypass, fallback-hemmeligheter, uforseglet PowerOffice ClientKey, manglende helmet/CSP/HSTS og CSRF, ingen tilpasset RLS, inkonsistent vendor-scoping og ingen 2FA/MFA | Vil bli funnet i enhver sikkerhetsrevisjon | Integrasjonsgrenen har lukket dev-bypass/fallback, transportkontroller, brede BOLA-flater, forseglet/roterbar PowerOffice ClientKey og RLS fase 1–2 for bekymringsmeldingskjernen og sikker dialog; produksjonshvelv, resten av RLS-matrisen, MFA, ekstern test og resterende endepunktsmatrise gjenstår | Resten må ryddes og kontrollene produksjonsbevises før tilbud på sensitiv sektor er troverdig |
