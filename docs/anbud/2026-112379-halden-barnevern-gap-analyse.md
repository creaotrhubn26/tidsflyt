...
| **G-10** | **Sikkerhetsherding** (uavhengig av anbudet): historisk dev-mode auth-bypass, fallback-hemmeligheter, uforseglet PowerOffice ClientKey, manglende helmet/CSP/HSTS og CSRF, ingen tilpasset RLS, inkonsistent vendor-scoping og ingen 2FA/MFA | Vil bli funnet i enhver sikkerhetsrevisjon | Integrasjonsgrenen har lukket dev-bypass/fallback, transportkontroller, brede BOLA-flater og forseglet/roterbar PowerOffice ClientKey; produksjonshvelv, tilpasset RLS, MFA, ekstern test og resterende endepunktsmatrise gjenstår | Resten må ryddes og kontrollene produksjonsbevises før tilbud på sensitiv sektor er troverdig |
