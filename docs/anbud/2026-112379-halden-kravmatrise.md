...
- SQL-injectionen i `PUT /api/report-templates/:id` er lukket på `main`
  (kolonne-whitelist, egen enhetstest). De tre gjenværende dynamiske
  CMS-oppdateringene (design_tokens/section_design_settings/page-version-
  restore) bygde SET-klausulen fra `Object.keys(...)` med kun blocklist og er
  nå flyttet til samme whitelist (`server/lib/design-token-columns.ts`).
  Rutene er `authenticateAdmin`-beskyttet.
...
|---:|---|---|---|---|---|
| 1 | Skal | Elektronisk/manuelt meldingsmottak, ufødt, tillegg, søskenkopi og redigering | Integrasjonsgrenen har manuelt API med mottakstid/kilde/kontakt, tenant-scope/RLS, vedlegg og avklaringsfrist, og nå også (migrasjon 088): prioritet akutt/normal med 24 t/7 d frist, ufødt barn med termindato (fnr avvist), tilleggsmelding som arver barnet og kjedes flatt til originalen, søskenkopi med eget barn, og kontrollert redigering med felt-whitelist, obligatorisk begrunnelse, append-only revisjonslogg og sperre etter avklaring — alt DB-testet. Maskinporten og kryptert rålogg finnes. | Delvis – komplett mottak med FIKS IO-grunnmur | Norsk UI levert. FIKS IO-mottaket er nå reelt (migrasjon 098): AMQP-abonnent med detaljer verifisert mot KS' åpne klientkode (kø fiksio.konto.<id>, Maskinporten-credentials), fail-closed persist av kryptert payload med konvoluttmetadata og idempotens på FIKS-meldings-id, ack først etter lagring, CMS-dekryptering (openssl, roundtrip-testet) og prosessering til bekymringsmelding med frist — gated på avtalt meldingstype og eksplisitt feltmapping (feltnavn gjettes aldri). Rest: KS-konto/sandkasse; DigiBarnevern-skjemaet som fyller mappingen; kvittering tilbake til avsender; E2E-bevis mot testmiljø. |
| 2 | Skal | Tilpassbar faseflyt fra mottak til avslutning | Integrasjonsgrenen har nå egen kommunal barnevernssak (migrasjon 087, FORCE RLS): «send til undersøkelse» oppretter sak transaksjonelt med saksnummer, tre måneders undersøkelsesfrist i fristmotoren, kodefaste overgangsregler (undersøkelse → tiltak/henlagt, tiltak → avsluttet), ledergodkjenning for avsluttende vedtak, append-only fasehistorikk og DB-testet prosess-/tenanttest. | Delvis – sakskjerne i integrasjonsgren | Norsk UI; per-kommune-konfigurerbar faseflyt; knytt journal/planer/vedtak til saken; flere fasetyper (akutt, ettervern); part/foreldreansvar; full akseptansetest. |
| 3 | Skal | Oppgave med eier, frist, varsel og eskalering | `main` har oppgaver/varsler. PR #21 legger til tildeling, frist, UI og eskaleringscron; fristmotor brukes for meldinger. Integrasjonsgrenen har nå migrasjon 086 med `FORCE RLS`, krav om nøyaktig én kommune/vendor, mottaker i samme tenant og tenantvis claim etter avgrenset systemscan. | Delvis – oppgaver og eskaleringsmatrise på barnevernsobjekter | Migrasjon 090 gir oppgaver med eier, frist, varsel og status (åpen/fullført/kansellert) på melding og sak, FORCE RLS, fristmotor-kobling og DB-testet tenant-/rolleisolasjon. Eskaleringsmatrisen i fristmotoren varsler nå barnevernsleder ved oversittelse av avklarings-, undersøkelses- og oppgavefrister. Rest: oppgave-UI; driftsalarm utover varsler; full E2E i produksjonsnær arkitektur. |
| 4 | Skal | Strukturert journal, fritekst, tid/forfatter og dokumenter | Integrasjonsgrenen har nå (migrasjon 089) append-only journal direkte på den kommunale barnevernssaken: kategorikodeliste, tid/forfatter, korreksjonskjede (ny rad, aldri UPDATE/DELETE — heller ikke som DB-grant), vedlegg med tilgangskontroll, FORCE RLS og DB-testet tenant-isolasjon. Utfører-sidens eldre journal med Documaster-kø og S3-vedlegg finnes fortsatt parallelt. | Delvis – kommunal journal med UI og arkivkø | Kommunal journal har nå norsk UI (/barnevern) og arkiv-outbox (entityType barnevern_journal) mot kommunens Documaster-/Elements-konfigurasjon, med meldingens saksmappe som arkivmål. Rest: norsk/EU-objektlager for vedlegg med dokumentert kryptering/nøkler; komplett tilgangs- og oppslaglogging (krav 15); sandkassebevis mot arkiv. |
| 5 | Skal | Tiltak-/planmodul med ansvar, dato, status og rapportering | `main` har § 6-3-tiltaksplan og evalueringsmal, saksperioder/status, rapportperioder, mål med status/fremdrift, aktivitetslogg, godkjenning og rapportering. Integrasjonsgrenen har tenant-/objektsikret dagens sak–rapport–mål–aktivitet-graf, inkludert DB-regel mot mål i feil rapport. Systemmal-seeding har en kjent feil på frisk database. | Delvis – versjonert planobjekt på kommunal sak | Migrasjon 092 gir autoritativt, versjonert planobjekt (tiltaksplan/omsorgsplan) på den kommunale saken: plandeltakere med rolle, tiltak med ansvar/frist/status for rapportering, evalueringsfrist i fristmotoren med ledereskalering, faglig godkjenning forbeholdt barnevernsleder, uforanderlige godkjente versjoner og ny-versjon-kopi — FORCE RLS og DB-testet. Utfører-sidens rapportbaserte § 6-3-flyt består parallelt. Rest: fiks utfører-seeding; plan-UI; vedtaks-/samtykkekobling (krav 6). |
| 6 | Skal | Malstyrte standardbrev/vedtak med forhåndsutfylling | Ni rapport-/planmaler, dynamiske felt, godkjenningsflyt, signaturfelt og PDF finnes i kildekoden. Frisk-database-seeding og enkelte malruter har åpne feil/sikkerhetsfunn. | Delvis – eget brev-/vedtaksobjekt på kommunal sak | Migrasjon 093 gir eget dokumentobjekt (vedtak/brev) skilt fra rapport: standardmaler med hjemmel, saksdatafletting ved opprettelse (malinnhold snapshotes — utstedte dokumenter påvirkes aldri av malendring), redigerbart utkast, godkjenning (vedtak krever barnevernsleder), ekspedering med mottaker som journalfører dokumentet og går i arkiv-outboxen; ekspedering via sikker dialog sender nå faktisk dokumentet som kryptert melding i sakens sikre samtale (gjenbruker åpen samtale eller oppretter ny med alle parter med aktiv tilgang, varsel i utboks, 409 uten part) — FORCE RLS og DB-testet. PDF-generering levert: GET /api/barnevern/dokumenter/:id/pdf gir nøktern brev-PDF (avsender, mottaker, hjemmel, godkjennings-/ekspederingslinje, konfidensiell bunntekst; utkast merkes UTKAST), behovsprøvd og auditlogget. SQL-injectionvern komplett: PUT /api/report-templates/:id og de tre dynamiske CMS-oppdateringene (design_tokens/section_design_settings/page-version-restore) bruker nå kolonne-whitelist (server/lib/design-token-columns.ts) i stedet for blocklist. Kommune-egne maler levert (migrasjon 104): leder oppretter/overstyrer/deaktiverer egne maler i DB, katalogen slår sammen kodefaste og egne, snapshot-garantien består (utstedte dokumenter urørt av malendring) — DB-testet. PDF-visningsknapper levert (PR #51). UI for maladministrasjon levert: «Maler»-fane (leder) med malkatalog (standard/kommunemal-merking) og opprett/oppdater/deaktiver-skjema med flettefelt-hjelp. Rest: fiks utfører-seeding; formell kodeverksliste for hjemler. |
...
| 9 | E | Kundens egen, leverandørnøytrale SMS-gateway | Tekst og telefonfelt finnes, men ingen SMS-gatewayimplementasjon ble funnet. | Delvis – adapter og utboks | Migrasjon 096 gir leverandørnøytralt gatewaygrensesnitt (generisk REST-adapter konfigurert via miljø; Haldens gateway plugges inn uten kodeendring), tenant-bundet utboks med FORCE RLS, claim-basert prosessering med eksponentiell backoff og terminal feilstatus, telefonnormalisering, obligatorisk formål per melding (personvernsporbarhet), reservasjonsstatusfelt (KRR-oppslag = ekstern rest) og lederinnsyn i utboksen uten meldingsinnhold — DB-testet med fake gateway. Rest: konfigurer og verifiser Haldens faktiske gateway; KRR-reservasjonsoppslag; leveringskvitteringer (DLR) om gatewayen tilbyr det. |
| 10 | Skal | Standardrapporter til ledelse, Bufdir/Statsforvalter og SSB; planlegging | Generiske rapporter og dashboards finnes. Ingen identifisert myndighetsrapportpakke eller planlagt innrapportering. | Delvis – autoritativt rapporteringsgrunnlag | Halvårsrapporteringsflaten (kun barnevernsleder) beregner kildetallene direkte fra saksdataene i én transaksjon: meldinger mottatt/akutte/ufødt/tillegg/henlagt/til undersøkelse/fristbrudd, undersøkelser startet/konkludert med utfall/fristbrudd/aktive over frist, bestand i undersøkelse/tiltak, godkjente planer og evalueringsfristbrudd, innsynskrav og forebyggende aktiviteter/deltakere — med kommune-/periodemetadata, DB-testet. Migrasjon 097 gir i tillegg daglig automatisk innrapportering til Barnevernsregisteret etter Flyt-/Modulus-modellen: dagsnapshot av hendelser og bestand snapshotes med SHA-256, KVALITETSSIKRES før innsending (avvik → «avvist» med feilliste, sendes aldri), sendes via konfigurerbar transport (BVR_API_URL/TOKEN — Bufdir-avtale er ekstern rest) med eksklusivt claim/backoff, kvittering lagres for avstemming, daglig 06:00-cron + timesretry, idempotent per kommune/dag (sendt datasett røres aldri), lederlogg og manuell trigger. Transporten er nå KS FIKS Protokoll (Bufdirs autoritative kanal): Maskinporten-autentisert (gjenbrukt klient), integrasjons-id/-passord mot FIKS IO, hybridkryptert payload (AES-256-GCM + RSA-OAEP) mot Barnevernsregisterets kontonøkkel, konfigurert via miljø og fail-closed uten oppsett — enhetstestet mot mocket FIKS-API inkl. krypteringsroundtrip. Rest: KS-konto/integrasjon og Maskinporten-klient i produksjon; Bufdirs XSD-payloadmapping og avtalt meldingstype; sandkassetest med kvittering før aktivering; leder-UI. |
...
| 12 | E | Ad hoc, CSV/Excel og rapporterings-API | CSV/Excel og flere API-er finnes, primært for tids-/rapportdata. Fiksen gjør egenbruk til standard og leder-`all` eksplisitt tenantavgrenset, samt nøytraliserer regnearkformler og HTML. | Delvis – barnevernsdatasett med masking og audit | CSV-eksport av meldings- og saksdatasett (kun barnevernsleder): obligatorisk periodefilter, fødselsnummer maskert til fødselsdato (DDMMYY*****), regnearkformler nøytralisert (gjenbrukt spreadsheetSafe), no-store, og hver eksport auditlogget med periode og radantall i tilgangsloggen — DB-testet inkl. rolle-/tenantsperre. Rest: flere datasett (journal/plan/tiltak); rapporterings-API; komplett systemomfattende BOLA-test. |
| 13 | E | Beskriv hvordan nøkkeltall hentes | Analysekomponenter og dashboards finnes for eksisterende domene. | Delvis – KPI-katalog med sporbar beregning | Ti barnevern-KPI-er er definert i kode med kilde, formel (den faktiske SQL-en eksponeres), eier og frekvens; alle beregnes i én transaksjon mot de autoritative saksdataene og vises i egen Nøkkeltall-fane for barnevernsleder med definisjonen synlig per kort — DB-testet inkl. tenant-isolasjon og rollesperre. Rest: datakvalitetsregler per KPI; historikk/trend; eksport til ledelsesrapport. |
| 14 | E | RBAC og juridisk avgrenset innsyn; minst tre roller | `main` har omfattende RBAC. PR #21 har kommune-tenant og to kommuneroller med DB-oppslag. Integrasjonsgrenen håndhever serveravledet tenant, tildeling/eierskap og egne redigerings-/godkjenningsrettigheter i sak–rapport–mål–aktivitet-grafen. Migrasjon 083–086 gir `FORCE RLS` for bekymringsmeldingskjernen, hele sikker-dialoggrafen, arkiv og frister. Arkiv og frister støtter både kommune og vendor; fristmottakeren må tilhøre samme tenant. Innbygger med BankID/Buypass er objektavgrenset til egen aktive parts-/samtalekjede, også mot andre parter i samme kommune. | Delvis – tre kommuneroller med need-to-know | Ny kommune_admin-rolle administrerer kommunens fagbrukere (rollebytte etter canManageRole-matrisen, aldri egen rolle/fremmed kommune/ikke-kommuneroller) men er eksplisitt utestengt fra ALLE saksflater (admin ≠ fag). Need-to-know på saksnivå: saksbehandler ser og behandler kun meldinger/saker tildelt seg selv eller utildelte; barnevernsleder ser alt; regelen håndheves i alle objektoppslag og arves av journal, planer, dokumenter, innsyn og oppgaver — DB-testet. Rest: delegasjon/fravær, nødtilgang (break-glass med logging), skjermet adresse, egen produksjonslogin og vendorflate-matrisen. |
| 15 | Skal | Alle saksendringer og dokumentoppslag logges søkbart | Flere append-/auditlogger finnes; PR #21 journal er append-only og BOLA på company audit er fikset. Rapport-audit, kommentarer, journal og vedlegg er nå objektskopet i hovedflyten. | Delvis – barnevernsflatene har lese-/nedlastingslogg | Migrasjon 091 gir append-only tilgangslogg (FORCE RLS, ingen DELETE/UPDATE-grant, overlever brukersletting) skrevet i samme transaksjon som lesingen for melding, sak, journal, revisjoner og vedleggsnedlasting, med søkbar revisorflate forbeholdt barnevernsleder — DB-testet. Rest: dekningskart og logging for øvrige saksobjekter utenfor barnevernskjernen; retensjonsvedtak for loggen; revisorrapport-eksport. Levert i migrasjon 102: fraværsdelegasjon (kun leder, obligatorisk begrunnelse, tidsavgrenset, opphevbar — rader bevares som tilgangsbevis), break-glass-nødtilgang (selvbetjent, én sak, 4 timer, høylytt auditlogget og synlig i leders liste) og skjermet adresse-markør på sak (følger utleverings-PDF). Alle hendelser i tilgangsloggen med ny handling 'endret'. UI levert: «Tilgang»-fane (leder) med søkbar revisorrapport (filter på objekttype/bruker), delegasjonsadministrasjon (opprett/opphev) og break-glass-oversikt; «Saksuttrekk (ZIP)»-knapp i saksvisningen. |
| 16 | E | Innsynsbegjæring, utskrift, journalkopi og klagedokumentasjon | Brukeren kan laste ned egne persondata; admin-eksport, anonymisering/sletting, PDF-generering og PII-maskering finnes. Ingen saksrettet innsyns-/klageprosess. | Delvis – innsynsflyt på barnevernssak | Migrasjon 094 gir full arbeidsflyt: mottak med partsstatus og 5-dagers behandlingsfrist i fristmotoren (ledereskalering), lederbeslutning innvilget/delvis/avslått med hjemlede unntak (fvl. § 19-sladding) og obligatorisk begrunnelse, automatisk journalføring av beslutningen, kontrollert utlevering (sikker dialog/utskrift/manuell) auditlogget mot krav 17-uttrekket — sikker dialog-kanalen sender nå faktisk utleveringsmelding (beslutning, unntak, begrunnelse) i sakens sikre samtale, 409 uten part med aktiv tilgang, og klageflyt med oversendelse til statsforvalteren — FORCE RLS og DB-testet. Sladdet utleverings-PDF levert: beslutningens unntak kan peke på konkrete journaloppføringer (journalEntryIds), og GET /api/barnevern/innsynskrav/:id/sladdet-pdf genererer PDF der unntatt tekst er FYSISK fraværende (kun sladdemarkør med hjemmel) — bevist i test ved dekoding av PDF-innholdet. Rest: UI; sikkerhetsherd de eldre GDPR-/eksport-rutene. |
| 17 | E | Enkelt komplett saksuttrekk til bruker/klient | GDPR-eksport samler flere brukerrelaterte tabeller; generiske CSV/Excel/PDF/JSON-eksporter og rapport-PDF finnes. Ingen komplett barnevernsmappe eller kontrollert partsutlevering. | Delvis – komplett saksuttrekk for barnevernssak | GET /api/barnevern/saker/:id/uttrekk gir komplett mappe i én konsistent transaksjon: sak, melding med revisjoner, fasehistorikk, journal, vedleggsmetadata, planer med tiltak, vedtak/brev og oppgaver, med manifest (antall per del, SHA-256 innholds-hash, no-store). Utlevering er forbeholdt barnevernsleder og auditlogges med hash i tilgangsloggen — DB-testet inkl. rollesperre og tenant-isolasjon. ZIP-pakke levert: GET /api/barnevern/saker/:id/uttrekk/pakke gir saksuttrekk.json + binærvedlegg fra objektlageret + manifest med SHA-256 per fil (interne lagringsnøkler lekker ikke; manglende binær noteres i manifestet i stedet for å velte pakken); kun barnevernsleder, auditlogget. Partsutlevering via sikker dialog levert i PR #36. Rest: ingen vesentlige. |
| 18 | E | Dokumentere og arkivere forebyggende arbeid | Generelle saker har type/status/start/slutt/tildeling; rapporter har mål, aktiviteter og maler; godkjente rapporter kan køes til Documaster. Ingen egen klassifikasjon eller arbeidsflyt for forebyggende arbeid. | Delvis – forebyggende struktur på plass | Migrasjon 095 gir egen, ikke barn-bundet struktur: prosjekt/program med kategori, samarbeidsparter, ansvarlig og status; append-only aktivitetslogg med deltakertall; aggregert statistikk per kategori/status og aktivitet/deltakere per år — FORCE RLS og DB-testet. Rest: faglig UI; dokumenter/vedlegg på tiltak; arkivmetadata/Noark-kobling for avsluttede tiltak. |
...
| 20 | Skal | Sentral identitet/SSO og MFA | Direkte BankID og Buypass ID er implementert for web/mobil. Sikker-dialoggrunnmuren forhåndsregistrerer en eID-only part uten e-postlogin og kobler BankID og Buypass til samme portalbruker; portalrollen er skilt fra ansatt-/leverandøradministrasjon. Entra ID SSO for kommuneansatte er på main (multi-tenant, invitasjonsbundet, inert til ENTRA_ID_CLIENT_ID/SECRET + kommunens tenant-id er satt). TOTP (G-10) er portert til main: innrullering med QR + engangs-viste gjenopprettingskoder (migrasjon 101, secret-box-forseglet hemmelighet, hash-lagrede koder), step-up ved HVER innlogging for admin-roller håndhevet i alle admin-guards (requireVendorAuth/requireSuperAdmin/requireAdminRole), 30-dagers nådeperiode fra TOTP_ROLLOUT_DATE, koblet inn i Google-, magic-link-, BankID/Buypass- og Entra-sporene. Ingen ID-porten. | Delvis – SSO + MFA implementert | Produksjonsbevis for BankID/Buypass; konfigurer Entra-appen og kommunens tenant-id; avklar skriftlig om Halden krever ID-porten for eksterne; livssyklus/SCIM ved behov; test mot kundetenanter. |
...
| 24 | Skal | Sikker loggforvaltning og definerte oppbevaringsperioder | Flere logger og audit-UI finnes. Sikker dialog har nå eksplisitt kommune-policy uten destruktiv standard, arkiv-før-sletting, juridisk sperring, retry og append-only slettingsbevis. Ny barnevern-retention-cron (server/lib/barnevern-retention.ts) minimerer teknisk sidedata etter konfigurerbare frister (SMS-innhold 90 d, prosessert FIKS-rålogg 90 d, varslede driftsalarmer 180 d) — saksdata slettes bevisst ALDRI automatisk (lovpålagt langtidsoppbevaring; kassasjon via arkivflyt). Øvrig loggeretensjon er fortsatt inkonsistent og ingen samlet SIEM-/tilgangsmodell er bevist. | Delvis | Vedta perioder med Halden; loggklassifisering, formål, samlet retention, tilgang/review, alarm/SIEM, bred slettetest og samsvar med arkiv/DPA. |
| 25 | Skal | SLA i Bilag 4 | Database-healthcheck, Render-healthprobe, maskert klient-Sentry, operasjonelle driftsmailer, backup-/restore-skript og runbook finnes. Det er ikke funnet produksjonsbevis for aktiv overvåking/backup/restore, og RPO står som 24 timer. | Delvis – driftsalarm for køfeil | Migrasjon 100 + server/lib/drift-alarm.ts: cron (hvert 5. min) oppdager kø-oppføringer i terminal feil (arkiv 'failed', SMS 'feilet', Barnevernsregisteret 'feilet'/'avvist'), dedupliserer per oppføring og sender samle-epost til DRIFT_ALARM_EPOST; uten mottaker logges alarmene og etterslepet varsles ved første konfigurasjon — DB-testet. Rest: Fyll Bilag 4; 99,5 %, 500 ms, RPO ≤2 t, utilgjengelighet ≤24 t, supportmål, kreditt; målbar monitorering, vakt, DR-, backup- og lasttest. |
