...
Migrasjon 086 innfører fase 3B for fristdomenet:

- `tidum_frister` har `ENABLE` + `FORCE RLS` for både kommune og vendor;
- hver frist må tilhøre nøyaktig én av de to tenanttypene;
- en varslingsmottaker må tilhøre samme tenant som fristen, håndhevet med
  sammensatte fremmednøkler;
- en bruker kan ikke samtidig være kommune- og vendorbruker;
- eskaleringsjobben bruker systemkontekst bare til søket, og claimer hver rad i
  radens egen tenantkontekst.

Den delte `users`-tabellen er bevisst ikke lagt bak generell RLS. Den er felles
identitetsregister for global innlogging, vendorinnlogging, kommuneansatte,
portalbrukere og BankID/Buypass. Fase 3B legger derfor integritetsregler på
tenantformen og fristmottakeren uten å endre innloggings- og
eID-provisioneringsflytene.

Dette er fase 1–3B, ikke en påstand om full systemomfattende RLS. Resten av
vendor-domenet og øvrige saksobjekter skal føres inn i en kontrollert
tabell-/endepunktsmatrise før RLS kan beskrives som komplett.
...
Migrasjon 083–086 bruker derfor PostgreSQLs innebygde `pg_database_owner`
transaksjonslokalt. Rollen er `NOLOGIN`, `NOSUPERUSER` og `NOBYPASSRLS`, og
migrasjonene gir bare eksplisitte rettigheter på de beskyttede tabellene og
...
migrasjon 083–086, ikke generelle rettigheter på alle eller fremtidige
tabeller.
...
2. kjør migrasjon 083, 084, 085 og 086 idempotent med migrasjonsidentiteten;
3. bekreft `relrowsecurity=true` og `relforcerowsecurity=true` på alle 19
   tabellene i fase 1–3B;
...
7. test opprettelse, tildeling, vedlegg, sikker dialog, varsling, karantene,
   retensjon, rått FIKS-inntak, kommune-/vendorarkivering, fristeskalering og
   nøkkelrotasjon;
8. kontroller at en tilkobling uten kontekst ser null beskyttede rader etter
   commit.

Utviklingsbevis 27.08.2026: migrasjon 083–086 er kjørt idempotent mot Neon.
Den siste samlede, berørte pakken besto 10 testfiler og 61/61 tester. Testene
...
dialog, varsling, karantene, retensjon, arkiv, fristmottakerbinding,
fristeskalering og nøkkelrotasjon.
...
- full tabell-/endepunktsmatrise for resten av kommunedomenet og relevante
  vendorflater, inkludert dokumentert vurdering av `users`-registeret uten å
  bryte global innlogging eller BankID/Buypass;
